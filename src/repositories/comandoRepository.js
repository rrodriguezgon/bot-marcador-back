...
const getAllComandos = async () => {
  return await Comando.find().catch((err) => {
    throw err;
  });
};
...
const getAllComandosWithFilters = async (filtersParams) => {
  ...
  return await Comando.find(filters).catch((err) => {
    throw err;
  });
};

const getComandoById = async (id) => {
  return await Comando.findById(id).catch((err) => {
    throw err;
  });
};
...
const updateComando = async (id, upcomando) => {
  return await Comando.findByIdAndUpdate(id, upcomando, {
    new: true,
...
const deleteComando = async (id) => {
  return await Comando.findByIdAndDelete(id).catch((err) => {
    throw err;
  });
};
